  ...
  return (
    <>
      ...
      <View
        ...
        {data.map((item, index) => (
          <View key={index} style={{ marginTop: index > 0 ? heightToDp(6.7) : 0 }}>
            <OrderConfirmCard
      ...
      {showIndicators && (
        <View style={[styles.indicatorView, { marginVertical: 20 }]}>
          <View style={styles.row}>
            <MultiButton image={contact} title={'Contact Artist'} btnStyle={{ backgroundColor: '#668C6A' }} />
            <MultiButton image={panic} title={'Panic'} btnStyle={{ backgroundColor: '#EC615B' }} />
          </View>
        </View>
      )}
    </>
